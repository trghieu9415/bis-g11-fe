...
type PayrollsYearMonthTableProps = {
	month: string;
	year: string;
};

export default function PayrollsYearMonthTable({ month, year }: PayrollsYearMonthTableProps) {
	...
	useEffect(() => {
		dispatch(fetchPayrollsByYearMonth(`${year}-${month}`));
		// dispatch(fetchPayrollsByYearMonth('2025-02'));
	}, [dispatch, month, year]);
	...
	return (
		<div>
			<CustomTable columns={columns} data={payrollsByYearMonth} hiddenColumns={hiddenColumns} />
